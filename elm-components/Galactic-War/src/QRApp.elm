...
type QRResponseMsg = QRDecodeSuccess String
...
port requestDecode : RequestObject -> Cmd msg
...
--------------------------------------------------------------------------------
type alias ResponseQRSuccessObject = { decoded_string : String }
port receiveDecode : ( ResponseQRSuccessObject -> msg ) -> Sub msg

responseQRSuccessToMsg : ResponseQRSuccessObject -> Msg
responseQRSuccessToMsg response = QRDecodeSuccess response.decoded_string
...
viewQRResult : Model -> Html Msg
viewQRResult =
  let
    outer_container = Html.div [ ]
    qr_result_attributes = [ Html.Attributes.id "qr-result" ]
